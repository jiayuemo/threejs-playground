...
export function generateHexagonGeometries(): Record<string, BoxGeometry> {
  ...
  for(let i = -10; i <= 10; i++) {
    for (let j = -10; j <= 10; j++) {
      nextGeoPosition = tileToPositon(i, j, simplex);
      // only add next hexagon to geometry if it is inside of a circle with radius 16
      if (nextGeoPosition.length() < 16) {
        nextGeo = hexGeometry(nextGeoPosition);
        if (nextGeoPosition.z > HEIGHT_CONSTANTS.SNOW) {
          hexagonGeometries.snow = mergeBufferGeometries([hexagonGeometries.snow, nextGeo]) as BoxGeometry;
          if (Math.random() > 0.4) {
            hexagonGeometries.snow = mergeBufferGeometries([hexagonGeometries.snow, snowmanSpheres(nextGeoPosition)]) as BoxGeometry;
          }
        } else if (nextGeoPosition.z > HEIGHT_CONSTANTS.STONE) {
          ...
          if (Math.random() > 0.8) {
            hexagonGeometries.stone = mergeBufferGeometries([hexagonGeometries.stone, stoneSphere(nextGeoPosition)]) as BoxGeometry;
          }
        } else if (nextGeoPosition.z > HEIGHT_CONSTANTS.DIRT) {
          ...
          if (Math.random() > 0.6 && hexagonGeometries.grass) {
            hexagonGeometries.grass = mergeBufferGeometries([hexagonGeometries.grass, treePyramids(nextGeoPosition)]) as BoxGeometry;
          }
        } else if (nextGeoPosition.z > HEIGHT_CONSTANTS.GRASS) {
          hexagonGeometries.grass = mergeBufferGeometries([hexagonGeometries.grass, nextGeo]) as BoxGeometry;
          if (Math.random() > 0.6) {
            hexagonGeometries.grass = mergeBufferGeometries([hexagonGeometries.grass, treePyramids(nextGeoPosition)]) as BoxGeometry;
          }
        } else if (nextGeoPosition.z > HEIGHT_CONSTANTS.SAND) {
        ...
        } else if (nextGeoPosition.z > HEIGHT_CONSTANTS.DIRT2) {
...
function hexGeometry(position: Vector3): CylinderGeometry {
  const geo = new CylinderGeometry(1,1, position.z, 6, 1, false);
  geo.translate(position.x, position.z * 0.5, position.y);
  return geo;
}

function tileToPositon(tileX: number, tileY: number, simplex: SimplexNoise): Vector3 {
  // X position: A * B
  ...
  const zCoord: number = Math.pow(
    (simplex.noise2D(tileX * 0.1, tileY * 0.1) + 1) * 0.5,
    1.5
  ) * MAX_HEIGHT;
